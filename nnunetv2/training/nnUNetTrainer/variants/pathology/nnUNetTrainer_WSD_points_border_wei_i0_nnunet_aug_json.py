...
class nnUNetTrainer_WSD_points_border_wei_i0_nnunet_aug_json(nnUNetTrainer_WSD_undefined_dataloader):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.iterator_template = f'wsd_{self.label_sampling_strategy}_point_border_iterator_{self.aug}_aug_json'


    def modify_fill_template(self, fill_template):
        point_to_seg_callback_name = "PointsToSegBatchCallbackInnerBorderTillCentroid"
        ...
        fill_template['batch_callbacks'][callback_idx]['centroid_size'] = self.dataset_json["centroid_size"] if "centroid_size" in self.dataset_json else 1.5
        fill_template['batch_callbacks'][callback_idx]['border_value'] = self.dataset_json["border_value"] if "border_value" in self.dataset_json else None